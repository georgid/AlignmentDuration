...
for i in range(len(scoreDev)):
    pyplot.scatter(i, noDurationError[i], marker='D', color='cyan', facecolors='none', s=100)
    pyplot.scatter(i, durationError[i], marker='D', color='red',  s=50)

    pyplot.plot([i, i], [noDurationError[i], durationError[i]],  linestyle='dotted', color='black'  )
    pyplot.plot([i, i], [noDurationError[i], durationErrorAcapella[i]],  linestyle='dotted', color='black'  )


#because of labeling
pyplot.scatter(i, noDurationError[i], marker='D', color='cyan', facecolors='none', s=100, label='HMM acapella')
pyplot.scatter(i, durationError[i], marker='D', color='red', s=50, label='VTHMM acapella')

pyplot.xlabel('number of recording (%)', fontsize=28)
...
fig1.savefig('/Users/joro/Documents/Phd/UPF/voxforge/myScripts/AlignmentDuration/ISMIR_noteOnsets/scatterCorrelation.svg')
